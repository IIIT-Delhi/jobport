...
class StudentForm(forms.ModelForm):
	...
	class Meta:
		model = Student
		fields=['resume', 'phone','email_personal','percentage_tenth','percentage_twelfth']
	...
	def clean_resume(self):
		...
		if resumeext in EXTENSIONS:
			if resume._size > MAX_UPLOAD_SIZE:
				raise forms.ValidationError('Please keep filesize under %s. Current filesize %s') % (filesizeformat(MAX_UPLOAD_SIZE), filesizeformat(resume._size))
		else:
...
class NewStudentForm(forms.ModelForm):
	...
	class Meta:
		model = Student
		exclude = ['user', 'email', 'companyapplications', 'status', 'placedat', 'cgpa_ug', 'cgpa_pg','name','createdon']
		widgets = { 'dob': DateTimePicker(options={"format": "YYYY-MM-DD", "pickTime": False}) }
...
class RootSearchForm(SearchForm):
